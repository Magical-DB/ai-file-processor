...
const processFromCommandLine = () => __awaiter(void 0, void 0, void 0, function* () {
    program
        ...
        .option("-ig, --ignoreList <ignorelist>", "Optional Files to ignore seperated by ,")
        .option("-gp, --globPattern <globPattern>", "Optional glob pattern for filtering file names e.g *.txt")
        .parse(process.argv);
    const options = program.opts();
    const openAiKey = options.key;
    const inputFolder = options.inputFolder;
    const globPattern = options.globPattern;
    const _ignoreList = options.ignoreList;
    let ignoreList = [];
    if (_ignoreList) {
    ...
    yield fileProcessor.processFilesInFolder(inputFolder, outputFileGenerator, instruction, {
